...
public class Calendar implements Selectable, Serializable {
    ...
    public void showDay(DateTime day) {
        ...
        for (int i = 0; i < events.size(); i++) {
            if (day.getDate().equals(events.get(i).getStartDate().getDate()))
    ...
    public void showWeek(DateTime startDate) {
        ...
        for (int i = 0; i <= 6; i++) {
            ...
            for (int j = 0; j < events.size(); j++) {
                if (date.getDate().equals(events.get(j).getStartDate().getDate()))
                    System.out.println(events.get(j).toString());
            }
            date = date.nextDay();
        }
    ...
    public void deleteEvent(DateTime day) {
        ...
        if (eventTime.isEmpty() || (!eventTime.isEmpty() && new DateTime(day.getDate(), eventTime).timeIsCorrect())) {
            for (int i = 0; i < events.size(); i++) {
    ...
    public boolean isEvent(int d, int m, int y) {
        for (int i = 0; i < events.size(); i++) {
            if (events.get(i).getStartDate().getDay() == d && events.get(i).getStartDate().getMonth() == m && events.get(i).getStartDate().getYear() == y)
                return true;
        }
        return false;
    }
